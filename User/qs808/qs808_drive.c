...
uint8_t QS808_Wait(uint32_t time)																											//修改好
{
	...
	while(cnt>0)
	{
		...
		else if(temp == ERR_SUCCESS)
				return ERR_SUCCESS;
		else
			return QS808_DATA_ERR;
//		if((temp == QS808_DATA_ERR) || (temp == QS808_UNKNOW_ERR))
...
uint8_t QS808_Login(uint8_t * ID)//参数是u8指针,用于输出指纹的ID,ID从1开始
//返回值：
...
//		printf("请按手指%d\n",i);
		if(i==0)
			Disp_sentence(20,2,"请按手指",1);
		else
			Disp_sentence(15,2,"请重复按手指",1);
		temp = QS808_Wait(10000);//采集指纹
		if(temp == QS808_WAIT_TIMEOUT)
			{
//			printf("等待超时\n");	
			return QS808_WAIT_TIMEOUT;
			}
			temp = QS808_CMD_GENERATE((uint16_t)(i<<8));//生成特征
...
//		uint8_t ID;
		temp = QS808_CMD_GET_EMPTY_ID(ID);//获取可注册的ID
		if((temp == QS808_DATA_ERR) || (temp == QS808_UNKNOW_ERR) || (temp == ERR_INVALID_PARAM))
				return QS808_DATA_ERR;
		temp = QS808_CMD_STORE_CHAR((uint16_t)(*ID<<8));//入库
...
uint8_t	QS808_CMD_DEL_NUM2(uint8_t index)//与QS808_CMD_DEL_NUM的区别在于 QS808_CMD_DEL_NUM 是按手指 然后删除。QS808_CMD_DEL_NUM2是根据指纹ID删除指纹
{	
	//清空固定编号的指纹
	uint16_t sum;
	int i;
